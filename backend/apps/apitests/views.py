...
class ApiTestCaseViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = ApiTestCase.objects.select_related("project", "created_by").all()
    ...
    def execute(self, request):
        ...
        if testcase_ids:
            testcases = ApiTestCase.objects.filter(id__in=testcase_ids, project_id=project_id, status="active")
        else:
            testcases = ApiTestCase.objects.filter(project_id=project_id, status="active")
        ...
        execution = ApiTestExecution.objects.create(
            ...
            name=f"接口测试执行 {timezone.now().strftime('%Y-%m-%d %H:%M:%S')}",
            total=len(testcases),
            executor=request.user,
        )

        if async_mode:
            task = run_api_tests_task.delay(execution.id, testcase_ids)
...
class ApiTestExecutionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = ApiTestExecution.objects.select_related("project", "executor").prefetch_related("results").all()
        project_id = self.request.query_params.get("project")
    ...
    def cancel(self, request, pk=None):
        ...
        return success_response(ApiTestExecutionSerializer(execution).data)
